...
from torch_geometric.utils import softmax, degree, is_undirected, to_undirected
...
class GNN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers,
        ...
        self.convs.append(My_GCN_Layer(in_channels, hidden_channels, cached=True, add_self_loops=False))
        self.bns = torch.nn.ModuleList()
        self.bns.append(torch.nn.BatchNorm1d(hidden_channels))
        for _ in range(num_layers - 2):
            self.convs.append(
                My_GCN_Layer(hidden_channels, hidden_channels, cached=True, add_self_loops = False))
            self.bns.append(torch.nn.BatchNorm1d(hidden_channels))
        self.convs.append(My_GCN_Layer(hidden_channels, out_channels, cached=True, add_self_loops=False))
    ...
    def forward(self, x, adj_t, adv_atts):
        for i, conv in enumerate(self.convs[:-1]):
            adv_att = adv_atts[i]
            x = conv(x, adj_t, adv_att)
            x = self.bns[i](x)
            x = F.relu(x)
            x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.convs[-1](x, adj_t, adv_atts[-1])
...
class My_GCN_Layer(GCNConv):
    def remove_cache(self):
        self._cached_edge_index = None

    def forward(self, node_inp, edge_index, adv_att, edge_weight=None):
        x = node_inp
        if self.normalize:
        ...
        out = self.propagate(edge_index, node_inp=x, adv_att=adv_att, edge_weight=edge_weight)
    ...
    def message(self, edge_index_i, node_inp_j, adv_att, edge_weight):
        '''
        ...
        return node_inp_j
#        return node_inp_j * self.att.view(-1, 1) if edge_weight is None\
#               else self.att.view(-1,1)*edge_weight.view(-1,1)*node_inp_j
...
def pre_process_no_batch(d):
    new_edge_index = to_undirected(d.edge_index)
    new_edge_index, _ = add_remaining_self_loops(new_edge_index)
    ori_edge_index = new_edge_index
    new_edge_index, init_att = randomly_perturb(node_size = len(d.x), edge_index = new_edge_index, ratio=0.2, undirected=True)
    return Data(x=d.x, y=d.y, edge_index=new_edge_index, edge_attr=init_att, ori_edge_index = ori_edge_index, batch=None,
...
def trades_on_edge(data, model, train = True, _type = 'l2'):
    ...
    model.eval()
    for i in range(10):
        ori_out = model(data.x, data.edge_index, original_adv_atts)
        adv_out = model(data.x, data.edge_index, [torch.clamp(ori_att + adv_att, min=0.0001)\
...
#pyg_data.edge_index, _ = add_remaining_self_loops(pyg_data.edge_index)
...
for epoch in range(args.num_epochs):
    ...
    for i in range(args.step_per_epoch):
        ...
        model.train()
        optimizer.zero_grad()
        ori_out = model(data.x, data.edge_index, ori_adv_atts)
        adv_out = model(data.x, data.edge_index, adv_adv_atts)
        ...
        if args.gvat_w > 0:
            ...
            loss_gvat = 0
        """
        loss_robust = criterion_kl(F.log_softmax(ori_out, dim=1), F.softmax(adv_out, dim=1))/data.num_nodes
    ...
    with torch.no_grad():
        ...
        model.eval()
        out = model(data.x, data.edge_index, ori_adv_atts)
...
with torch.no_grad():
    ...
    for ratio in args.n_perturbations:
        ...
        #wrp_pyg_data.data.edge_index = to_undirected(wrp_pyg_data.data.edge_index)
        #wrp_pyg_data.data.edge_index, _ = add_remaining_self_loops(wrp_pyg_data.data.edge_index)

        adv_data = pre_process_no_batch(wrp_pyg_data.data).to(device)
        ori_adv_atts, _ = trades_on_edge(adv_data, model, train = False)
        #print(adv_data.edge_index.shape)
        #print(ori_adv_atts[0].shape)
        model.remove_cache()
        out = model(adv_data.x, adv_data.edge_index, ori_adv_atts)
