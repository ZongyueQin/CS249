...
class GGCL_F(Module):
    ...
    def forward(self, features, adj_norm1, adj_norm2, gamma=1):
        ...
        if utils.is_sparse_tensor(adj_norm1):
            miu_out = torch.spmm(adj_norm1, (self.miu*Att))
            sigma_out = torch.spmm(adj_norm2, (self.sigma*Att*Att))
        else:
...
class GGCL_D(Module):
    ...
    def forward(self, miu, sigma, adj_norm1, adj_norm2, gamma=1):
        ...
        if utils.is_sparse_tensor(adj_norm1):
            mean_out = torch.spmm(adj_norm1, (miu*Att))
            sigma_out = torch.spmm(adj_norm2, (sigma*Att*Att))
        else:
...
class RGCN(Module):
    ...
    def __init__(self, nnodes, nfeat, nhids, nclass, gamma=1.0, beta1=5e-4, beta2=5e-4, lr=0.01, dropout=0.6, device='cpu'):
        ...
        self.gc1 = GGCL_F(nfeat, nhids[0]//2, dropout=dropout)
        #self.gc2 = GGCL_D(nhid, nclass, dropout=dropout)
        self.nouts  = nhids[1:] + [nclass*2]
        self.dgc = nn.ModuleList([GGCL_D(nhids[i]//2, self.nouts[i]//2, dropout=dropout) for i in range(len(nhids))])
    ...
    def forward(self):
        ...
        for dgc in self.dgc:
            miu, sigma = dgc(miu, sigma, self.adj_norm1, self.adj_norm2, self.gamma)
...
#        adj, features, labels = utils.to_tensor(adj.todense(), features.todense(), labels, device=self.device)
        ...
        is_adj_sparse = (adj.layout == torch.sparse_coo)
        if not is_adj_sparse:
            print('Warning: adj is not sparse')
        self.features, self.labels = features, labels
        self.adj_norm1 = self._normalize_adj(adj, power=-1/2, sparse=is_adj_sparse)
        self.adj_norm2 = self._normalize_adj(adj, power=-1, sparse=is_adj_sparse)
    ...
    def predict(self, features = None, adj = None):
        ...
        if features is not None:
            if type(features) is not torch.Tensor:
                adj, self.features = utils.to_tensor(adj, features, device = self.device)
            else:
                self.features = features.to(self.device)
                adj = adj.to(self.device)
#        if adj is not None:
#            if type(adj) is not torch.Tensor:
#                adj = utils.to_tensor(adj, device=self.device)
#            else:
        is_adj_sparse = (adj.layout == torch.sparse_coo)
        if not is_adj_sparse:
            print('Warning: adj is not sparse')
    ...
    def _normalize_adj(self, adj, power=-1/2, sparse = False):
        ...
        if not sparse:
            ...
            mx = r_mat_inv.dot(mx)
            mx = mx.dot(r_mat_inv)
            return utils.sparse_mx_to_torch_sparse_tensor(mx).to(self.device)
